...
if (!defined('PWD_MIN_LENGTH')) {
    define('PWD_MIN_LENGTH', 3); // Todo Passwd Min auf 8
}
...
class PasswordableBehavior extends ModelBehavior {
    ...
    protected $_validationRules = array(
        'formField' => array(
            'between' => array(
                ...
                'message' => array('Das Passwort sollte aus %s bis %s Zeichen bestehen.', PWD_MIN_LENGTH, PWD_MAX_LENGTH),
                'allowEmpty' => null,
                'last' => true,
            )
        ),
    ...
    public function validateCurrentPwd(Model $Model, $data) {
        ...
        if ($Model->id) {
            $uid = $Model->id;
        } elseif (!empty($Model->data[$Model->alias]['user_id'])) {
            $uid = $Model->data[$Model->alias]['user_id'];
        } else {
        ...
        $authConfig = array(
            'fields' => array('username' => 'user_id', 'password' => $this->settings[$Model->alias]['field']),
        ...
        $request->data[$Model->alias] = array('user_id' => $uid, 'password' => $pwd);
