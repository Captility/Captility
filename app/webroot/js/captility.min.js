$mobileMaxWidth=992;function getParameterByName(b,f){b=b.replace(/[\[]/,"\\[").replace(/[\]]/,"\\]");var d=RegExp("[\\?&]"+b+"=([^&#]*)").exec(f);return null==d?"":decodeURIComponent(d[1].replace(/\+/g," "))}jQuery.fn.slideRight=function(b,f){$(this).css({width:"0"});var d,a;return this.each(function(n,c){c=jQuery(c);d=c.clone().css({width:"auto"}).appendTo(c.parent());a=d.css("width");d.remove();c.animate({width:a},b,f)})};$(".panel-collapse  a").click(function(b){b.stopPropagation()});
...
$("#calendar").fullCalendar("changeView","agendaWeek")});var f={url:$appRoot+"events/feed",type:"GET",cache:!1},d={url:$appRoot+"events/feedMy",type:"GET",cache:!1};$('a[data-toggle="tab"]').on("shown.bs.tab",function(a){$("#calendar").fullCalendar("render");$("#calendar").fullCalendar("render")});$("#GeneralViewFc").click(function(){$(this).parent().hasClass("active")||$("#calendar").fullCalendar("removeEventSource",d).fullCalendar("removeEventSource",f).fullCalendar("addEventSource",f)});$("#MyWeekViewFc").click(function(){$(this).parent().hasClass("active")||
$("#calendar").fullCalendar("removeEventSource",f).fullCalendar("removeEventSource",d).fullCalendar("addEventSource",d)});$("#calendar").fullCalendar({header:$mediaQueryHeader,defaultView:$mediaQueryView,weekMode:"variable",allDaySlot:!1,weekends:!0,firstDay:1,hiddenDays:$hiddenDays,aspectRatio:1.215,firstHour:7,minTime:6,maxTime:22,axisFormat:"HH:mm",formatDate:"dd.MM.yyyy HH:mm:ss",titleFormat:{month:"MMMM yyyy",week:"dd.[ MMM.][yy]{ '&#8211;' dd. MMMM yyyy}",day:"dddd, dd.MM.yyyy"},columnFormat:{month:"ddd",
...
editable:!1,selectable:!0,viewRender:function(a,c){$("#SideCalendar").datepicker("update",new Date(a.start))},eventClick:function(a,c,d){a.place="H12";d='<ul class="list-group"><li><span class="glyphicon glyphicon-calendar"></span>'+a.datec+' <li><span class="glyphicon glyphicon-time gl-ml"></span>'+a.time+" "+(a.place&&'  <span class="glyphicon glyphicon-map-marker gl-ml"></span>'+a.place+"</li><br/>"||"")+(a.niceEnd&&"<p><b>End:</b> "+a.end+"</p>"||"")+'<li><span class="glyphicon glyphicon-th-list"></span><a href="lectures/view/'+
...
a.capture_id+'">Anzeigen';console.log(a);b.set({"content.text":d,"content.title":'<span class="glyphicon glyphicon-film"></span>'+a.title,"style.classes":"qtip-bootstrap qtip-"+a.className}).reposition(c).show(c)},eventResizeStart:function(){b.hide()},eventDragStart:function(){b.hide()},viewDisplay:function(){b.hide()},eventDrop:function(a){var c=new Date(a.start),b=c.getFullYear(),d=c.getDate(),f=c.getMonth()+1,g=c.getHours(),c=c.getMinutes(),e=new Date(a.end),h=e.getFullYear(),k=e.getDate(),l=e.getMonth()+
1,m=e.getHours(),e=e.getMinutes();$.post($appRoot+"events/update?id="+a.id+"&start="+b+"-"+f+"-"+d+" "+g+":"+c+":00&end="+h+"-"+l+"-"+k+" "+m+":"+e+":00&allday="+(!0==a.allDay?1:0),function(a){})},eventResize:function(a){var c=new Date(a.start),b=c.getFullYear(),d=c.getDate(),f=c.getMonth()+1,g=c.getHours(),c=c.getMinutes(),e=new Date(a.end),h=e.getFullYear(),k=e.getDate(),l=e.getMonth()+1,m=e.getHours(),e=e.getMinutes();$.post($appRoot+"events/update?id="+a.id+"&start="+b+"-"+f+"-"+d+" "+g+":"+c+
":00&end="+h+"-"+l+"-"+k+" "+m+":"+e+":00",function(a){})}});if($("#calendar").length){var a=getParameterByName("date",window.location.href);a&&($("#calendar").fullCalendar("changeView","agendaDay"),$("#calendar").fullCalendar("gotoDate",new Date(a)))}}});var eventColorValues={colors:["#202020 #3a87ad #f70 #009406 #FFEB00 #FD3E20 #5F43A8 #009B9B #579B00 #FF38D7 #FDFDFD".split(" ")]},eventColorNames="Black Blue Orange Green Yellow Red Purple Indigo Mint Pink White".split(" ");
$(document).ready(function(){function b(){$.isFunction($.fn.sortable)&&$(".workflow-task-list").sortable({forcePlaceholderSize:!0,items:":not(.sort-disabled)"}).bind("sortupdate",function(){})}$(".colorpalette").colorPalette(eventColorValues).on("selectColor",function(a){$(".selected-color").val(a.color)});$("select:not(.form-control-date)").addClass("show-tick").selectpicker({selectedTextFormat:"values",noneSelectedText:'<span class="glyphicon el-icon-error"></span>'});$("select.form-control-date").selectpicker({selectedTextFormat:"values",
noneSelectedText:'<span class="glyphicon el-icon-error"></span>',width:"auto"});$(document).keydown(function(a){if($(event.target).is("input, textarea, select"))return!0;if(37==a.keyCode||65==a.keyCode)return $(".fc-button-prev").click(),!0;if(39==a.keyCode||68==a.keyCode)return $(".fc-button-next").click(),!0;if(32==a.keyCode||83==a.keyCode||87==a.keyCode)return $(".fc-button-today").click(),!0});(function(a){a.fn.updateScheduleRemoveState=function(){var b=a("#ScheduleContainer button.form-schedule-remove");
1<b.length?b.prop("disabled",!1):b.prop("disabled",!0);console.log(b.length);return!0}})(jQuery);$("#ScheduleContainer").on("shown.bs.tab","a.form-toggle",function(a){a=$($(this).attr("href"));a.parent().find("input, select").prop("disabled",!0).selectpicker("refresh");a.find("input, select").prop("disabled",!1).selectpicker("refresh")});$(function(){var a=window.location.hash;a&&$('ul.nav a[href="'+a+'"]').first().tab("show");$(".nav-tabs a").click(function(a){a=$("body").scrollTop();window.location.hash=
this.hash;$("html,body").scrollTop(a)})});if($.isFunction($.fn.datepicker))$("#ScheduleContainer").on("click","button.form-schedule-add",function(){var a=$("#ScheduleContainer .panel").length;$("#ScheduleContainer").append('<div class="panel panel-default"> <div class="panel-heading panel-link" data-toggle="collapse" href="#container'+a+'"> <small class="glyphicon glyphicon-time"></small> <strong>Aufnahmezeit einstellen</strong></div> <div class="panel-body" id="container'+a+'"> <ul class="nav nav-tabs nav-justified tabs-left"> <li class="active"><a href="#single'+
...
'"><span class="glyphicon glyphicon-link"></span>'+window.location.href.substring(0,30)+"...</a>",button:"Close"},show:{event:"click"},style:"qtip-bootstrap",position:{my:"top right",at:" top right"},events:{show:function(a,b){var c=window.location.href;$(this).find(".qr-code-container").empty().qrcode({width:250,height:250,text:c});console.log(c)}},hide:{event:"unfocus",effect:function(){$(this).animate({opacity:0},{duration:300})}}});jQuery.fn.switchTaskToEditMode=function(){$(this).find(".task-view").hide();
$(this).find(".task-edit").show().find("input, textarea").each(function(){$(this).prop("disabled",!1)});return $(this)};jQuery.fn.switchTaskToViewMode=function(){$(this).find(".task-edit").hide().find("input, textarea").each(function(){});$(this).find(".task-view").show();return $(this)};jQuery.fn.countTasksFrom=function(a){console.log($(a).find("li:not(.task-template, .workflow-task-add)").length);return $(a).find("li:not(.task-template, .workflow-task-add)").length};jQuery.fn.updateTaskId=function(a){$(this).find("label, textarea, input").each(function(){for(var b=
["name","for","id"],c=0;c<b.length;++c)if(void 0!==$(this).prop(b[c])){var d=$(this).prop(b[c]).replace("$placeholder$",a);$(this).prop(b[c],d)}});return $(this)};b();$(".workflow-task-list").sortable({forcePlaceholderSize:!0,items:":not(.sort-disabled)"}).bind("sortupdate",function(){});$(".workflow-task-list").on("click",".workflow-task-add",function(){var a=$(this).parent().find("li.task-template");if(0<a.length){var d=a.clone().removeClass("task-template").hide().addClass("task");d.switchTaskToEditMode();
d.updateTaskId(d.countTasksFrom(a.parent()));a.before(d);d.slideDown("800");b()}})});
