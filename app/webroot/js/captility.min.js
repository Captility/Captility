...
editable:!1,selectable:!0,viewRender:function(c,a){$("#SideCalendar").datepicker("update",new Date(c.start))},eventClick:function(c,a,b){b='<ul class="list-group"><li><span class="glyphicon glyphicon-calendar"></span>'+c.datec+' <li><span class="glyphicon glyphicon-time gl-ml"></span>'+c.time;null!=c.location&&(b+=(c.location&&'  <span class="glyphicon glyphicon-map-marker gl-ml"></span>'+c.location+"</li><br/>"||"")+(c.niceEnd&&"<p><b>End:</b> "+c.end+"</p>"||""));null!=c.Lecture.lecture_id&&(b+=
'<li><span class="glyphicon glyphicon-th-list"></span><a href="lectures/view/'+c.Lecture.lecture_id+'">'+c.Lecture.number+" "+c.Lecture.name+"</a>"+(c.Lecture.link&&'<a href="'+c.Lecture.link+'"> <span class="glyphicon glyphicon-link gl-ms"></span></a>')+"</li><br/>");null!=c.Host.host_id&&(b+='<li><span class="glyphicon cp-icon-lecturer"></span><a href="hosts/view/'+c.Host.host_id+'">'+c.Host.name+"</a>"+(c.Host.email&&'<a href="mailto:'+c.Host.email+'"> <span class="glyphicon glyphicon-envelope gl-ms"></span></a>')+
...
eventDrop:function(c){var a=new Date(c.start),b=a.getFullYear(),m=a.getDate(),d=a.getMonth()+1,e=a.getHours(),a=a.getMinutes(),g=new Date(c.end),f=g.getFullYear(),h=g.getDate(),k=g.getMonth()+1,l=g.getHours(),g=g.getMinutes();$.post($appRoot+"events/update?id="+c.id+"&start="+b+"-"+d+"-"+m+" "+e+":"+a+":00&end="+f+"-"+k+"-"+h+" "+l+":"+g+":00&allday="+(!0==c.allDay?1:0),function(a){})},eventResize:function(c){var a=new Date(c.start),b=a.getFullYear(),m=a.getDate(),d=a.getMonth()+1,e=a.getHours(),
a=a.getMinutes(),g=new Date(c.end),f=g.getFullYear(),h=g.getDate(),k=g.getMonth()+1,l=g.getHours(),g=g.getMinutes();$.post($appRoot+"events/update?id="+c.id+"&start="+b+"-"+d+"-"+m+" "+e+":"+a+":00&end="+f+"-"+k+"-"+h+" "+l+":"+g+":00",function(a){})}});if($("#calendar").length){var h=getParameterByName("date",window.location.href);h&&($("#calendar").fullCalendar("changeView","agendaDay"),$("#calendar").fullCalendar("gotoDate",new Date(h)))}}});
...
"slow").slideUp(1E3);$(this).dequeue()});$("#ScheduleContainer").on("click","button.form-schedule-add",function(){var a=$("#ScheduleContainer .panel-body").last();a.hide();$("html, body").animate({scrollTop:a.parent().offset().top},800,function(){a.slideDown(800)})});var f=$(".container-wrapper").first(),h=$(".btn-scrollTop").first();f.on("click","button.btn-scrollTop",function(){$("html, body").animate({scrollTop:$("body").offset().top},600)});h.length&&h.offset().top<$(window).height()&&h.hide();
$("img.landing-page-thumbnail").click(function(){$(".modal-body").empty();var a=$(this).parent("a").attr("title");a?($(".modal-title").html(a),$("#LandingPageModal").find(".modal-dialog").css({"max-width":652})):($(".modal-title").html("Captility unterst\u00fctzt alle Ger\u00e4te!"),$("#LandingPageModal").find(".modal-dialog").css({"max-width":this.naturalWidth+52}));$($(this).parents("div").html()).appendTo(".modal-body");$("#LandingPageModal").modal({show:!0})});$.isFunction($.fn.unveil)&&$("img").unveil();
$(".captility-breadcrumb li").each(function(a){$(this).css({"z-index":255-a})});$(".captility-breadcrumb li").last().hide().css({"margin-left":"-400px"}).show().animate({"margin-left":"0"},600);$.isFunction($.fn.qrcode)&&$("a.qr-code").qtip({id:"qr-code",prerender:!0,content:{text:'<div class="qr-code-container"></div>',title:'<a href="'+window.location.href+'"><span class="glyphicon glyphicon-link"></span>'+window.location.href.substring(0,30)+"...</a>",button:"Close"},show:{event:"click"},style:"qtip-bootstrap",
position:{my:"top right",at:" top right"},events:{show:function(a,b){var c=window.location.href;$(this).find(".qr-code-container").empty().qrcode({width:250,height:250,text:c})}},hide:{event:"unfocus",effect:function(){$(this).animate({opacity:0},{duration:300})}}});jQuery.fn.switchTaskToEditMode=function(){$(this).find(".task-view").hide();$(this).find(".task-edit").show().find("input, textarea").each(function(){$(this).prop("disabled",!1)});return $(this)};jQuery.fn.switchTaskToViewMode=function(){$(this).find(".task-edit").slideUp().find("input, textarea").each(function(){});
