...
function(d){$("#calendar").length?$("#calendar").fullCalendar("gotoDate",new Date(d.date)):window.location.href=$appRoot+"/calendars/?date="+d.date})});
...
editable:!1,selectable:!0,viewRender:function(b,a){$("#SideCalendar").datepicker("update",new Date(b.start))},eventClick:function(b,a,c){b.place="H12";d.set({"content.text":'<ul class="list-group"><li><span class="glyphicon glyphicon-calendar"></span>'+b.datec+' <li><span class="glyphicon glyphicon-time gl-ml"></span>'+b.time+" "+(b.place&&'  <span class="glyphicon glyphicon-map-marker gl-ml"></span>'+b.place+"</li><br/>"||"")+(b.niceEnd&&"<p><b>End:</b> "+b.end+"</p>"||"")+'<li><span class="glyphicon glyphicon-th-list"></span><a href="lectures/view/'+
...
$(document).ready(function(){function d(a,c,b){c.last().nextAll();b=cssprop(a.first().prev(),"margin-bottom");isNaN(b)&&(b=0);var d=cssprop(c.last().next(),"margin-top");isNaN(d)&&(d=0);var e=cssprop(a.first(),"margin-top"),g=cssprop(a.last(),"margin-bottom"),f=cssprop(c.first(),"margin-top"),h=cssprop(c.last(),"margin-bottom"),k=a.last().offset().top+a.last().outerHeight()-a.first().offset().top,l=c.last().offset().top+c.last().outerHeight()-c.first().offset().top;move1=l+Math.max(h,e)+Math.max(b,
f)-Math.max(b,e);move2=-k-Math.max(g,f)-Math.max(b,e)+Math.max(b,f);move3=move1+a.first().offset().top+k-c.first().offset().top-l+Math.max(g,d)-Math.max(h,d);a.css("position","relative");c.css("position","relative");a.animate({top:move1},{duration:800});c.animate({top:move2},{duration:800,complete:function(){a.insertAfter(c.last());a.removeAttr("style");c.removeAttr("style");a.updateTaskStep()}})}function e(){$.isFunction($.fn.sortable)&&$(".workflow-task-list").sortable({forcePlaceholderSize:!0,
items:":not(.sort-disabled)"}).bind("sortupdate",function(a,c){c.item.updateTaskStep()})}$(".colorpalette").colorPalette(eventColorValues).on("selectColor",function(a){$(".selected-color").val(a.color)});$.isFunction($.fn.selectpicker)&&($("select:not(.form-control-date)").addClass("show-tick").selectpicker({selectedTextFormat:"values",noneSelectedText:'<span class="glyphicon el-icon-error"></span>',noneResultsText:'<span class="glyphicon el-icon-remove-sign"></span>',dropupAuto:!1,liveSearch:!0}),
