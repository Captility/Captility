$mobileMaxWidth=992;function getParameterByName(c,e){c=c.replace(/[\[]/,"\\[").replace(/[\]]/,"\\]");var a=RegExp("[\\?&]"+c+"=([^&#]*)").exec(e);return null==a?"":decodeURIComponent(a[1].replace(/\+/g," "))}jQuery.fn.slideRight=function(c,e){$(this).css({width:"0"});var a,d;return this.each(function(n,b){b=jQuery(b);a=b.clone().css({width:"auto"}).appendTo(b.parent());d=a.css("width");a.remove();b.animate({width:d},c,e)})};
...
$(document).ready(function(){if($.isFunction($.fn.datepicker)&&$.isFunction($.fn.fullCalendar)){if($.isFunction($.fn.qtip))var c=$("<div/>").qtip({id:"calendar",prerender:!0,content:{text:" ",title:{button:!0}},position:{my:"top center",at:" center",target:"mouse",viewport:$("#calendar"),adjust:{mouse:!1,scroll:!1}},show:!1,hide:{event:"unfocus click mouseleave",effect:function(){$(this).animate({opacity:0},{duration:300})}},style:"qtip-bootstrap"}).qtip("api");$mediaQueryHeader={left:"title",center:"",
...
"agendaDay"):$("#calendar").fullCalendar("changeView","agendaWeek")});var e={url:$appRoot+"events/feed",type:"GET",cache:!1},a={url:$appRoot+"events/feedMy",type:"GET",cache:!1};$('a[data-toggle="tab"]').on("shown.bs.tab",function(a){$("#calendar").fullCalendar("render");$("#calendar").fullCalendar("render")});$("#GeneralViewFc").click(function(){$(this).parent().hasClass("active")||$("#calendar").fullCalendar("removeEventSource",a).fullCalendar("removeEventSource",e).fullCalendar("addEventSource",
e)});$("#MyWeekViewFc").click(function(){$(this).parent().hasClass("active")||$("#calendar").fullCalendar("removeEventSource",e).fullCalendar("removeEventSource",a).fullCalendar("addEventSource",a)});$("#calendar").fullCalendar({header:$mediaQueryHeader,defaultView:$mediaQueryView,weekMode:"variable",allDaySlot:!1,weekends:!0,firstDay:1,hiddenDays:$hiddenDays,aspectRatio:1.215,firstHour:8,minTime:6,maxTime:22,axisFormat:"HH:mm",formatDate:"dd.MM.yyyy HH:mm:ss",titleFormat:{month:"MMMM yyyy",week:"dd.[ MMM.][yy]{ '&#8211;' dd. MMMM yyyy}",
...
week:"Woche",day:"Tag"},eventSources:[e],editable:!1,selectable:!0,viewRender:function(a,b){$("#SideCalendar").datepicker("update",new Date(a.start))},eventClick:function(a,b,d){a.place="H12";d='<ul class="list-group"><li><span class="glyphicon glyphicon-calendar"></span>'+a.datec+' <li><span class="glyphicon glyphicon-time gl-ml"></span>'+a.time+" "+(a.place&&'  <span class="glyphicon glyphicon-map-marker gl-ml"></span>'+a.place+"</li><br/>"||"")+(a.niceEnd&&"<p><b>End:</b> "+a.end+"</p>"||"")+'<li><span class="glyphicon glyphicon-th-list"></span><a href="lectures/view/'+
...
a.capture_id+'">Anzeigen';console.log(a);c.set({"content.text":d,"content.title":'<span class="glyphicon glyphicon-film"></span>'+a.title,"style.classes":"qtip-bootstrap qtip-"+a.className}).reposition(b).show(b)},eventResizeStart:function(){c.hide()},eventDragStart:function(){c.hide()},viewDisplay:function(){c.hide()},eventDrop:function(a){var b=new Date(a.start),d=b.getFullYear(),c=b.getDate(),e=b.getMonth()+1,g=b.getHours(),b=b.getMinutes(),f=new Date(a.end),h=f.getFullYear(),k=f.getDate(),l=f.getMonth()+
1,m=f.getHours(),f=f.getMinutes();$.post($appRoot+"events/update?id="+a.id+"&start="+d+"-"+e+"-"+c+" "+g+":"+b+":00&end="+h+"-"+l+"-"+k+" "+m+":"+f+":00&allday="+(!0==a.allDay?1:0),function(a){})},eventResize:function(a){var b=new Date(a.start),d=b.getFullYear(),c=b.getDate(),e=b.getMonth()+1,g=b.getHours(),b=b.getMinutes(),f=new Date(a.end),h=f.getFullYear(),k=f.getDate(),l=f.getMonth()+1,m=f.getHours(),f=f.getMinutes();$.post($appRoot+"events/update?id="+a.id+"&start="+d+"-"+e+"-"+c+" "+g+":"+b+
":00&end="+h+"-"+l+"-"+k+" "+m+":"+f+":00",function(a){})}});if($("#calendar").length){var d=getParameterByName("date",window.location.href);d&&($("#calendar").fullCalendar("changeView","agendaDay"),$("#calendar").fullCalendar("gotoDate",new Date(d)))}}});var eventColorValues={colors:["#202020 #3a87ad #f70 #009406 #FFEB00 #FD3E20 #5F43A8 #009B9B #579B00 #FF38D7 #FDFDFD".split(" ")]},eventColorNames="Black Blue Orange Green Yellow Red Purple Indigo Mint Pink White".split(" ");
...
a.duration).animate({opacity:1},a.duration).delay(50);return $(this)};$(".alert, .message").css({opacity:0}).animate({opacity:1},600).delay(3500);$(this).queue(function(){$(".alert-success, .alert-info").animate({opacity:"0"},"slow").slideUp(1E3);$(this).dequeue()});$("#ScheduleContainer").on("click","button.form-schedule-add",function(){var a=$("#ScheduleContainer .panel-body").last();a.hide();$("html, body").animate({scrollTop:a.parent().offset().top},800,function(){a.slideDown(800)})});var c=$(".container-wrapper").first(),
e=$(".btn-scrollTop").first();c.on("click","button.btn-scrollTop",function(){$("html, body").animate({scrollTop:$("body").offset().top},600)});e.length&&e.offset().top<$(window).height()&&e.hide();$("img.landing-page-thumbnail").click(function(){$(".modal-body").empty();var a=$(this).parent("a").attr("title");a?($(".modal-title").html(a),$("#LandingPageModal").find(".modal-dialog").css({"max-width":652})):($(".modal-title").html("Captility unterst\u00fctzt alle Ger\u00e4te!"),$("#LandingPageModal").find(".modal-dialog").css({"max-width":this.naturalWidth+
