...
week:"Woche",day:"Tag"},eventSources:[c],editable:!0,selectable:!0,viewRender:function(a,b){$("#SideCalendar").datepicker("update",new Date(a.start))},eventClick:function(a,n,c){a.place="H12";c='<ul class="list-group"><li><span class="glyphicon glyphicon-calendar"></span>'+a.datec+' <li><span class="glyphicon glyphicon-time gl-ml"></span>'+a.time+" "+(a.place&&'  <span class="glyphicon glyphicon-map-marker gl-ml"></span>'+a.place+"</li><br/>"||"")+(a.niceEnd&&"<p><b>End:</b> "+a.end+"</p>"||"")+'<li><span class="glyphicon glyphicon-th-list"></span><a href="lectures/view/'+
...
c=$(".btn-scrollTop").first();b.on("click","button.btn-scrollTop",function(){$("html, body").animate({scrollTop:$("body").offset().top},600)});c.length&&c.offset().top<$(window).height()&&c.hide();$("img.landing-page-thumbnail").click(function(){$(".modal-body").empty();var a=$(this).parent("a").attr("title");a?($(".modal-title").html(a),$("#LandingPageModal").find(".modal-dialog").css({"max-width":652})):($(".modal-title").html("Captility unterst\u00fctzt alle Ger\u00e4te!"),$("#LandingPageModal").find(".modal-dialog").css({"max-width":this.naturalWidth+
52}));$($(this).parents("div").html()).appendTo(".modal-body");$("#LandingPageModal").modal({show:!0})});$.isFunction($.fn.unveil)&&$("img").unveil()});
