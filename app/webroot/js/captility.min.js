...
function(c){$("#calendar").length?$("#calendar").fullCalendar("gotoDate",new Date(c.date)):window.location.href=$appRoot+"/calendars/?date="+c.date})});
...
editable:!1,selectable:!0,viewRender:function(b,a){$("#SideCalendar").datepicker("update",new Date(b.start))},eventClick:function(b,a,d){b.place="H12";c.set({"content.text":'<ul class="list-group"><li><span class="glyphicon glyphicon-calendar"></span>'+b.datec+' <li><span class="glyphicon glyphicon-time gl-ml"></span>'+b.time+" "+(b.place&&'  <span class="glyphicon glyphicon-map-marker gl-ml"></span>'+b.place+"</li><br/>"||"")+(b.niceEnd&&"<p><b>End:</b> "+b.end+"</p>"||"")+'<li><span class="glyphicon glyphicon-th-list"></span><a href="lectures/view/'+
b.Lecture.lecture_id+'">'+b.Lecture.number+" "+b.Lecture.name+"</a>"+(b.Lecture.link&&'<a href="'+b.Lecture.link+'"> <span class="glyphicon glyphicon-link gl-ms"></span></a>')+'</li><br/><li><span class="glyphicon cp-icon-lecturer"></span><a href="hosts/view/'+b.Host.host_id+'">'+b.Host.name+"</a>"+(b.Host.email&&'<a href="mailto:'+b.Host.email+'"> <span class="glyphicon glyphicon-envelope gl-ms"></span></a>')+(b.Host.contact_email&&'<a href="mailto:'+b.Host.contact_email+'"><span class="glyphicon glyphicon-envelope gl-ms"></span></a>')+
'</li><br/><li><span class="glyphicon glyphicon-facetime-video"></span>'+b.EventType.name+'</li><br/><li><span class="glyphicon glyphicon-user"></span><a href="users/view/'+b.User.user_id+'">'+b.User.username+'</a><a href="mailto:'+b.User.email+'"> <span class="glyphicon glyphicon-envelope gl-ms"></span></a></li><br/></ul><a class="btn-m btn-sm btn-default pull-right" name="Bearbeiten" href="captures/edit/'+b.capture_id+'">Bearbeiten<a class="btn-m btn-sm btn-default pull-right" name="Anzeigen" href="captures/view/'+
b.capture_id+'">Anzeigen',"content.title":'<span class="glyphicon glyphicon-film"></span>'+b.title,"style.classes":"qtip-bootstrap qtip-"+b.className}).reposition(a).show(a)},eventResizeStart:function(){c.hide()},eventDragStart:function(){c.hide()},viewDisplay:function(){c.hide()},eventDrop:function(b){var a=new Date(b.start),d=a.getFullYear(),c=a.getDate(),m=a.getMonth()+1,e=a.getHours(),a=a.getMinutes(),g=new Date(b.end),f=g.getFullYear(),h=g.getDate(),k=g.getMonth()+1,l=g.getHours(),g=g.getMinutes();
$.post($appRoot+"events/update?id="+b.id+"&start="+d+"-"+m+"-"+c+" "+e+":"+a+":00&end="+f+"-"+k+"-"+h+" "+l+":"+g+":00&allday="+(!0==b.allDay?1:0),function(a){})},eventResize:function(b){var a=new Date(b.start),d=a.getFullYear(),c=a.getDate(),e=a.getMonth()+1,f=a.getHours(),a=a.getMinutes(),g=new Date(b.end),h=g.getFullYear(),n=g.getDate(),k=g.getMonth()+1,l=g.getHours(),g=g.getMinutes();$.post($appRoot+"events/update?id="+b.id+"&start="+d+"-"+e+"-"+c+" "+f+":"+a+":00&end="+h+"-"+k+"-"+n+" "+l+":"+
g+":00",function(a){})}});if($("#calendar").length){var h=getParameterByName("date",window.location.href);h&&($("#calendar").fullCalendar("changeView","agendaDay"),$("#calendar").fullCalendar("gotoDate",new Date(h)))}}});var eventColorValues={colors:["#202020 #3a87ad #f70 #009406 #FFEB00 #FD3E20 #5F43A8 #009B9B #579B00 #FF38D7 #FDFDFD".split(" ")]},eventColorNames="Black Blue Orange Green Yellow Red Purple Indigo Mint Pink White".split(" ");
$(document).ready(function(){function c(a,d,b){d.last().nextAll();b=cssprop(a.first().prev(),"margin-bottom");isNaN(b)&&(b=0);var c=cssprop(d.last().next(),"margin-top");isNaN(c)&&(c=0);var e=cssprop(a.first(),"margin-top"),g=cssprop(a.last(),"margin-bottom"),f=cssprop(d.first(),"margin-top"),h=cssprop(d.last(),"margin-bottom"),k=a.last().offset().top+a.last().outerHeight()-a.first().offset().top,l=d.last().offset().top+d.last().outerHeight()-d.first().offset().top;move1=l+Math.max(h,e)+Math.max(b,
f)-Math.max(b,e);move2=-k-Math.max(g,f)-Math.max(b,e)+Math.max(b,f);move3=move1+a.first().offset().top+k-d.first().offset().top-l+Math.max(g,c)-Math.max(h,c);a.css("position","relative");d.css("position","relative");a.animate({top:move1},{duration:800});d.animate({top:move2},{duration:800,complete:function(){a.insertAfter(d.last());a.removeAttr("style");d.removeAttr("style");a.updateTaskStep()}})}function e(){$.isFunction($.fn.sortable)&&$(".workflow-task-list").sortable({forcePlaceholderSize:!0,
items:":not(.sort-disabled)"}).bind("sortupdate",function(a,d){d.item.updateTaskStep()})}$(".colorpalette").colorPalette(eventColorValues).on("selectColor",function(a){$(".selected-color").val(a.color)});$("select:not(.form-control-date)").addClass("show-tick").selectpicker({selectedTextFormat:"values",noneSelectedText:'<span class="glyphicon el-icon-error"></span>'});$("select.form-control-date").selectpicker({selectedTextFormat:"values",noneSelectedText:'<span class="glyphicon el-icon-error"></span>',
width:"auto"});$(document).keydown(function(a){if($(event.target).is("input, textarea, select"))return!0;if(37==a.keyCode||65==a.keyCode)return $(".fc-button-prev").click(),!0;if(39==a.keyCode||68==a.keyCode)return $(".fc-button-next").click(),!0;if(32==a.keyCode||83==a.keyCode||87==a.keyCode)return $(".fc-button-today").click(),!0});(function(a){a.fn.updateScheduleRemoveState=function(){var d=a("#ScheduleContainer button.form-schedule-remove");1<d.length?d.prop("disabled",!1):d.prop("disabled",!0);
return!0}})(jQuery);$("#ScheduleContainer").on("shown.bs.tab","a.form-toggle",function(a){a=$($(this).attr("href"));a.parent().find("input, select").prop("disabled",!0).selectpicker("refresh");a.find("input, select").prop("disabled",!1).selectpicker("refresh")});$(function(){var a=window.location.hash;a&&$('ul.nav a[href="'+a+'"]').first().tab("show");$(".nav-tabs a").click(function(a){a=$("body").scrollTop();window.location.hash=this.hash;$("html,body").scrollTop(a)})});if($.isFunction($.fn.datepicker))$("#ScheduleContainer").on("click",
...
'"><span class="glyphicon glyphicon-link"></span>'+window.location.href.substring(0,30)+"...</a>",button:"Close"},show:{event:"click"},style:"qtip-bootstrap",position:{my:"top right",at:" top right"},events:{show:function(a,d){var b=window.location.href;$(this).find(".qr-code-container").empty().qrcode({width:250,height:250,text:b})}},hide:{event:"unfocus",effect:function(){$(this).animate({opacity:0},{duration:300})}}});jQuery.fn.switchTaskToEditMode=function(){$(this).find(".task-view").hide();
$(this).find(".task-edit").show().find("input, textarea").each(function(){$(this).prop("disabled",!1)});return $(this)};jQuery.fn.switchTaskToViewMode=function(){$(this).find(".task-edit").slideUp().find("input, textarea").each(function(){});$(this).find(".task-view").fadeIn();return $(this)};jQuery.fn.countTasksFrom=function(a){return $(a).find("li:not(.task-template, .workflow-task-add)").length};jQuery.fn.updateTaskId=function(a){$(this).find("label, textarea, input").each(function(){for(var b=
["name","for","id"],c=0;c<b.length;++c)if(void 0!==$(this).prop(b[c])){var e=$(this).prop(b[c]).replace("$placeholder$",a);$(this).prop(b[c],e)}});return $(this)};jQuery.fn.updateTaskStep=function(){$(".workflow-task-list li.task").each(function(a,b){$(this).find(".task-step-field").find("input").val(a)});return $(this)};e();var b=["primary","warning","danger","info"];$(".workflow-task-list").on("click",".workflow-task-add",function(){var a=$(this).parent().find("li.task-template");if(0<a.length){var d=
a.clone().removeClass("task-template").hide().addClass("task");d.find(".timeline-badge").addClass(b[d.countTasksFrom(a.parent())%4]);d.switchTaskToEditMode();d.updateTaskId(d.countTasksFrom(a.parent()));a.before(d);d.slideDown("800");e();d.updateTaskStep()}});$("ul.workflow-task-list").on("click",".task-save",function(){var a=$(this).parents("li.task"),b=a.find("h4.task-name"),c=a.find(".task-description"),e=a.find(".task-name-field").find("input"),f=a.find(".task-description-field").find("textarea");
""===e.val()?e.closest(".form-group").addClass("has-error"):(b.find("hr").before(e.val()),c.html(f.val()),a.switchTaskToViewMode())});$("ul.workflow-task-list").on("click",".task-delete",function(){var a=$(this).parents("li.task");a.slideUp("800",function(){a.remove()})});$("ul.workflow-task-list").on("click",".task-config",function(){$(this).parents("li.task").switchTaskToEditMode()});$("ul.workflow-task-list").on("click",".task-down",function(){var a=$(this).parents("li.task"),b=a.next("li.task:not(.task-template, .workflow-task-add)");
b.length&&c(a,b,a.updateTaskStep())});$("ul.workflow-task-list").on("click",".task-up",function(){var a=$(this).parents("li.task"),b=a.prev("li.task:not(.task-template, .workflow-task-add)");b.length&&c(b,a)})});
