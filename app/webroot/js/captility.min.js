$mobileMaxWidth=992;function getParameterByName(b,d){b=b.replace(/[\[]/,"\\[").replace(/[\]]/,"\\]");var a=RegExp("[\\?&]"+b+"=([^&#]*)").exec(d);return null==a?"":decodeURIComponent(a[1].replace(/\+/g," "))}$(function(){var b=window.location.hash;b&&$('ul.nav a[href="'+b+'"]').first().tab("show");$(".nav-tabs a").click(function(b){b=$("body").scrollTop();window.location.hash=this.hash;$("html,body").scrollTop(b)})});
jQuery.fn.slideRight=function(b,d){$(this).css({width:"0"});var a,f;return this.each(function(p,c){c=jQuery(c);a=c.clone().css({width:"auto"}).appendTo(c.parent());f=a.css("width");a.remove();c.animate({width:f},b,d)})};
...
$(document).ready(function(){if($.isFunction($.fn.datepicker)&&$.isFunction($.fn.fullCalendar)){if($.isFunction($.fn.qtip))var b=$("<div/>").qtip({id:"calendar",prerender:!0,content:{text:" ",title:{button:!0}},position:{my:"top center",at:" center",target:"mouse",viewport:$("#calendar"),adjust:{mouse:!1,scroll:!1}},show:!1,hide:{event:"unfocus click mouseleave",effect:function(){$(this).animate({opacity:0},{duration:300})}},style:"qtip-bootstrap"}).qtip("api");$mediaQueryHeader={left:"title",center:"",
right:"prev,today,next"};$mediaQueryView="agendaWeek";$hiddenDays=[0,6];if(-1<window.location.href.indexOf("calendar")||-1<window.location.href.indexOf("full_calendar"))$mediaQueryHeader.left="agendaDay,agendaWeek,month",$mediaQueryHeader.center="title",$mediaQueryHeader.right="prev,today,next",$mediaQueryView="month",$hiddenDays=[0];$(document).width()<$mobileMaxWidth&&($mediaQueryView="agendaDay");$(window).bind("resize",function(){$(document).width()<$mobileMaxWidth?$("#calendar").fullCalendar("changeView",
"agendaDay"):$("#calendar").fullCalendar("changeView","agendaWeek")});var d={url:$appRoot+"events/feed",type:"GET",cache:!1},a={url:$appRoot+"events/feedMy",type:"GET",cache:!1};$('a[data-toggle="tab"]').on("shown.bs.tab",function(a){$("#calendar").fullCalendar("render");$("#calendar").fullCalendar("render")});$("#GeneralViewFc").click(function(){$(this).parent().hasClass("active")||$("#calendar").fullCalendar("removeEventSource",a).fullCalendar("removeEventSource",d).fullCalendar("addEventSource",
d)});$("#MyWeekViewFc").click(function(){$(this).parent().hasClass("active")||$("#calendar").fullCalendar("removeEventSource",d).fullCalendar("removeEventSource",a).fullCalendar("addEventSource",a)});$("#calendar").fullCalendar({header:$mediaQueryHeader,defaultView:$mediaQueryView,weekMode:"variable",allDaySlot:!1,weekends:!0,firstDay:1,hiddenDays:$hiddenDays,aspectRatio:1.215,firstHour:8,minTime:6,maxTime:22,axisFormat:"HH:mm",formatDate:"dd.MM.yyyy HH:mm:ss",titleFormat:{month:"MMMM yyyy",week:"dd.[ MMM.][yy]{ '&#8211;' dd. MMMM yyyy}",
day:"dddd, dd.MM.yyyy"},columnFormat:{month:"ddd",week:"ddd - d.M.",day:"dddd d.M."},timeFormat:"HH:mm{ \u2013 HH:mm}' Uhr'",monthNames:"Januar Februar M\u00e4rz April Mai Juni Juli August September Oktober November Dezember".split(" "),monthNamesShort:"Jan Feb Mrz Apr Mai Jun Jul Aug Sept Okt Nov Dez".split(" "),dayNames:"Sonntag Montag Dienstag Mittwoch Donnerstag Freitag Samstag".split(" "),dayNamesShort:"So Mo Di Mi Do Fr Sa".split(" "),allDayText:"Ganzt\u00e4gig",buttonText:{today:"Heute",month:"Monat",
week:"Woche",day:"Tag"},eventSources:[d],editable:!1,selectable:!0,viewRender:function(a,c){$("#SideCalendar").datepicker("update",new Date(a.start))},eventClick:function(a,c,f){a.place="H12";f='<ul class="list-group"><li><span class="glyphicon glyphicon-calendar"></span>'+a.datec+' <li><span class="glyphicon glyphicon-time gl-ml"></span>'+a.time+" "+(a.place&&'  <span class="glyphicon glyphicon-map-marker gl-ml"></span>'+a.place+"</li><br/>"||"")+(a.niceEnd&&"<p><b>End:</b> "+a.end+"</p>"||"")+'<li><span class="glyphicon glyphicon-th-list"></span><a href="lectures/view/'+
...
a.capture_id+'">Anzeigen';console.log(a);b.set({"content.text":f,"content.title":'<span class="glyphicon glyphicon-film"></span>'+a.title,"style.classes":"qtip-bootstrap qtip-"+a.className}).reposition(c).show(c)},eventResizeStart:function(){b.hide()},eventDragStart:function(){b.hide()},viewDisplay:function(){b.hide()},eventDrop:function(a){var c=new Date(a.start),b=c.getFullYear(),f=c.getDate(),d=c.getMonth()+1,g=c.getHours(),c=c.getMinutes(),e=new Date(a.end),h=e.getFullYear(),k=e.getDate(),l=e.getMonth()+
1,m=e.getHours(),e=e.getMinutes();$.post($appRoot+"events/update?id="+a.id+"&start="+b+"-"+d+"-"+f+" "+g+":"+c+":00&end="+h+"-"+l+"-"+k+" "+m+":"+e+":00&allday="+(!0==a.allDay?1:0),function(a){})},eventResize:function(a){var b=new Date(a.start),f=b.getFullYear(),d=b.getDate(),n=b.getMonth()+1,g=b.getHours(),b=b.getMinutes(),e=new Date(a.end),h=e.getFullYear(),k=e.getDate(),l=e.getMonth()+1,m=e.getHours(),e=e.getMinutes();$.post($appRoot+"events/update?id="+a.id+"&start="+f+"-"+n+"-"+d+" "+g+":"+b+
":00&end="+h+"-"+l+"-"+k+" "+m+":"+e+":00",function(a){})}});if($("#calendar").length){var f=getParameterByName("date",window.location.href);f&&$("#calendar").fullCalendar("gotoDate",new Date(f))}}});var eventColorValues={colors:["#202020 #3a87ad #f70 #009406 #FFEB00 #FD3E20 #5F43A8 #009B9B #579B00 #FF38D7 #FDFDFD".split(" ")]},eventColorNames="Black Blue Orange Green Yellow Red Purple Indigo Mint Pink White".split(" ");
...
52}));$($(this).parents("div").html()).appendTo(".modal-body");$("#LandingPageModal").modal({show:!0})});$.isFunction($.fn.unveil)&&$("img").unveil();$(".captility-breadcrumb li").each(function(a){$(this).css({"z-index":255-a})});$(".captility-breadcrumb li").last().hide().css({"margin-left":"-400px"}).show().animate({"margin-left":"0"},600)});
